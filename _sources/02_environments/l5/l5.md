...
but only to a limited extent in the continents. 
...
The latter idea that the continents, somehow, move with respect to one another is principally associated with Alfred Wegener (1880-1930). Early evidence was largely geological, with people observing similarities
...
and then spreads laterally, pushing the continents aside as it does so.
...
basaltic ocean floor which is comparatively rich in magnetic minerals), and
their **remnant magnetism** can be measured. Careful study 
...
from the ocean ridges, and, moreover, provided a quantitative means 
...
Subsequent observations covering the entirety of the ocean 
...
later understand one of the principal forces that drives plate motion. 
...
will have cooled and become denser. The density increase is approximately $\Delta\rho_{l} = 
\rho_{m} \alpha (T_{m} - T_{l})$, with $\alpha$
...
is limited to thin regions around the plate boundaries. Importantly, plate tectonics is a kinematic theory that explains the manner in which 
...
graphically the constraint that the relative motions of 
 ...
 surface can be described by rotation about an axis
...
such as the Himalayas. Taken from [the BGS](https://www.bgs.ac.uk/discovering-geology/earth-hazards/earthquakes/where-do-earthquakes-occur/).
```
...
models can be developed, but we will just summarise the 
basic ideas. 
...
Clearly these are not settled matters. From the perspective of a planetary scientist musing about the interior dynamics of an exoplanet
it is, however, important to know and remember that the behaviour of the 
Earth's mantle is not fully understood and cannot be self-consistently modelled. This should serve as  a note of caution for those inclined to run complicated numerical convection codes within a new parameter 
