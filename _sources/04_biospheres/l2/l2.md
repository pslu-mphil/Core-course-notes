...
The goal is to map the journey from simple molecules to protocells ({numref}`fig:01_molecular_to_cell`). This involves identifying:
...
Living organisms can be broken down into four key subsystems ({numref}`fig:02_subsystem`):
...
- The photo reduction of CO<sub>2</sub> using an aqueous solution of bicarbonate and sulfite under UV irradiation. Converting CO<sub>2</sub> into various organic compounds (formate, hydroxymethanesulfonate, methanol, glycolate, acetate, tartronate, malonate, _rac- and meso-_ tartrate, and hydrogen gas)({numref}`fig:03_CO2_reduction`).
...
Amino acids are the building blocks of proteins and enzymes, which are crucial for biological functions such as catalysis, structure, and signaling ({numref}`fig:04_catalog_amino_acids`).
...
---
name: fig:04_catalog_amino_acids
figclass: margin-caption
---
...
Breslow discovered the autocatalytic cycle in Butlerov's sugar synthesis in the mid-20th century. The C2 sugar, glycolaldehyde, acts as an autocatalyst in the synthesis of longer sugars ({numref}`fig:05_sugars`).
