...
Though, other possible explanations existed, such as specialisation of a region of cell membrane, followed by enclosure and capture of relevant genes ({numref}`fig:c4_8_specialisation`).
...
Sequence analysis of DNA indicates a close relationship with alpha-proteobacteria, and possibly even within them ({numref}`fig:c4_8_sequence_analysis`) though some analyses disagree.
...
It was thought they are representatives of eukaryote lineages diverging before the origin of mitochondria ({numref}`fig:c4_8_mitos`):
...
figclass: margin-caption
---
Phylogeny for mitochondria originating from amitochondriate eukaryotes.
```
...
Recent studies have indicated the existence of a group of archaea, the Asgardarchaeota, which have many features in common with eukaryotes. They group as a sister to eukaryotes in phylogenetic trees ({numref}`fig:c4_8_mitos2`).
...
figclass: margin-caption
---
Eukaryotes as sister to Asgardarchaeota.  Phylogenies from [Rodriguez-Oliveira et al. (2023)](https://doi.org/10.1038/s41586-022-05550-y).
```
...
Sequence-based trees indicate an origin of plastids in the cyanobacteria ({numref}`fig:c4_8_plastids1`).
...
figclass: margin-caption
---
Phylogeny for the origin of plastids.
```
...
The general view is that the primary plastids have a single origin – “monophyletic” (left hand panel below), rather than “polyphyletic” (right hand panel) in spite of the differences in pigment composition ({numref}`fig:c4_8_plastids2`). This is based on the plastids apparently having a common import machinery.
...
figclass: margin-caption
---
Monophyletic (left) or polyphyletic (right) phylogenies for the origins of plastids.
```

The complex plastids arose by secondary endosymbiosis, in which a non-photosynthetic host acquired a photosynthetic eukaryote, and kept the plastid ({numref}`fig:c4_8_tree_arrows`).
...
figclass: margin-caption
---
The eukaryotic tree of life, annotated to indicate primary (green arrows) and secondary (red arrows) endosymbiotic events (modified after [Faktorova et al., 2020](https://doi.org/10.1038/s41592-020-0796-x)).
```
...
Recently, an additional primary endosymbiosis has been recognised in the chromatophore of _Paulinella chromatophora_ ({numref}`fig:c4_8_tree_arrows2`).
...
More dramatically, the sea slug _Elysia_ ({numref}`fig:c4_8_slug`) can contain large numbers of plastids from the algae it eats, making it green.
